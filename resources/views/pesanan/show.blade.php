...
@section('content')
    ...
    <nav class="container py-3">
        <ol class="breadcrumb mb-0">
            ...
            <li class="breadcrumb-item"><a href="{{ route('pesanan.index') }}" class="text-decoration-none">Pesanan Saya</a>
            </li>
    ...
    <section class="py-5">
        ...
        default => $pesanan->status
                    ...
                    {{-- Info Cards: Pembayaran, Pengiriman, Penerima --}}
                            ...
                            <div class="card border-0 bg-light h-100">
                                <div class="card-body p-3">
                                    ...
                                    <p class="mb-0 small fw-bold">
                                        {{ $paymentLabels[$pesanan->metode_pembayaran] ?? $pesanan->metode_pembayaran }}
                                    </p>
                                    @if($pesanan->midtrans_payment_type)
                                        <p class="mb-0 small text-muted">via {{ strtoupper($pesanan->midtrans_payment_type) }}
                                        </p>
                                    @endif
                                </div>
                            </div>
                                ...
                                <div class="card-body p-3">
                                    ...
                                    @if($pesanan->kurir)
                                        <p class="mb-0 small text-muted mt-1">
                                            <i class="bi bi-truck me-1"></i>{{ strtoupper($pesanan->kurir) }} -
                                            {{ $pesanan->layanan_kurir }}
                                        </p>
                                    @endif
                                </div>
                            </div>
                                ...
                                <div class="card-body p-3">
                                    ...
                                    @if($pesanan->no_telepon)
                                        <p class="mb-0 small text-muted">
                                            <i class="bi bi-telephone me-1"></i>{{ $pesanan->no_telepon }}
                                        </p>
                                    @endif
                                    ...
                                    <tfoot>
                                        ...
                                        @if($pesanan->ongkir > 0)
                                            <tr>
                                                ...
                                                <td class="text-end fw-bold">Rp
                                                    {{ number_format($pesanan->ongkir, 0, ',', '.') }}</td>
                                            </tr>
                                        @else
                                            <tr>
                                                <td colspan="3" class="text-end text-muted">Ongkos Kirim:</td>
                                        ...
                                        @if($pesanan->diskon_voucher > 0)
                                            <tr>
                                                ...
                                                <td class="text-end text-success fw-bold">- Rp
                                                    {{ number_format($pesanan->diskon_voucher, 0, ',', '.') }}</td>
                                            </tr>
                                        @endif
                                        <tr class="border-top">
                                            ...
                                            <td class="text-end fw-bold fs-5 text-primary">
                                                Rp
                                                {{ number_format($pesanan->total_bayar ?? $pesanan->total, 0, ',', '.') }}
                                            </td>
                                        </tr>
                                    </tfoot>
                                </table>
                    ...
                    {{-- Midtrans: Retry Bayar --}}
                    @if($pesanan->status === 'pending' && $pesanan->metode_pembayaran === 'midtrans')
                        <div class="card border-0 shadow-sm mb-4 border-start border-warning border-4" data-aos="fade-up">
                            <div class="card-body p-4">
                                ...
                                <a href="{{ route('pesanan.bayar-midtrans', $pesanan) }}" class="btn btn-warning fw-bold">
                    ...
                    {{-- Instruksi Pembayaran & Batalkan (non-midtrans pending) --}}
                    @if($pesanan->status === 'pending' && $pesanan->metode_pembayaran !== 'midtrans')
                                ...
                                <div class="card-body p-4">
                                    ...
                                    @if($pesanan->bukti_pembayaran)
                                        <div class="text-center">
                                            <img src="{{ Storage::disk('public')->url($pesanan->bukti_pembayaran) }}"
                                    ...
                                    @else
                                        <form action="{{ route('pesanan.upload.bukti', $pesanan) }}" method="POST"
                            ...
                            <div class="timeline">
                                <div class="d-flex gap-3 mb-3">
                                    ...
                                    <div>
                                        ...
                                        <p class="mb-0 text-muted small">{{ $pesanan->created_at->format('d M Y, H:i') }}
                                        </p>
                                    </div>
                                </div>
                                @if(in_array($pesanan->status, ['diproses', 'dikirim', 'selesai']))
                                    <div class="d-flex gap-3 mb-3">
                                        ...
                                        <div>
                                            ...
                                            <p class="mb-0 text-muted small">{{ $pesanan->updated_at->format('d M Y, H:i') }}
                                            </p>
                                        </div>
                                    </div>
                                @endif
                    ...
                    @if($pesanan->status === 'selesai')
                        <div class="card border-0 shadow-sm mb-4" data-aos="fade-up">
                            <div class="card-body p-4">
                                ...
                                @foreach($pesanan->detailTransaksi as $detail)
                                    @php $existingReview = $pesanan->reviews->where('produk_id', $detail->produk_id)->first(); @endphp
                                    <div class="border rounded-3 p-3 mb-3 {{ $existingReview ? 'bg-light' : '' }}">
                                        <div class="d-flex align-items-center gap-3 mb-3">
                                            @if($detail->produk && $detail->produk->gambar)
                                                <img src="{{ $detail->produk->imageUrl() }}" class="rounded-2"
                                                    style="width:60px;height:60px;object-fit:cover;" alt="{{ $detail->nama_produk }}">
                                            @else
                                            ...
                                            @endif
                                            <div>
                                                <h6 class="fw-bold mb-0">{{ $detail->nama_produk }}</h6>
                                                <small class="text-muted">Rp {{ number_format($detail->harga_satuan, 0, ',', '.') }}
                                                    x {{ $detail->jumlah }}</small>
                                            </div>
                                        </div>
                                        @if($existingReview)
                                            <div class="bg-white rounded-3 p-3">
                                                <div class="d-flex align-items-center gap-1 mb-1">
                                                    @for($i = 1; $i <= 5; $i++)
                                                        <i class="bi bi-star-{{ $i <= $existingReview->rating ? 'fill text-warning' : 'text-muted' }}"
                                                    ...
                                                    <div class="rating-input d-flex gap-1" data-produk="{{ $detail->id }}">
                                                        @for($i = 1; $i <= 5; $i++)
                                                            <button type="button" class="btn btn-outline-warning btn-sm rating-star p-2"
                                                                data-rating="{{ $i }}" data-produk="{{ $detail->id }}">
